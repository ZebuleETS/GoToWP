...
from trajectory import StraightLineTrajectory, TrajectoryEvaluator, generate_all_trajectories, generate_random_obstacles
...
thermal_generated = len(active_thermals) # 3
#test du soaring
...
for u in range(nUAVs):
    ...
    FLT_conditions[u]['flight_path_angle'] = 0.0 # en radians
    FLT_conditions[u]['grav_accel'] = grav_accel
    FLT_conditions[u]['bank_angle'] = 0.0 # en radians
    ...
    FLT_track[u]['bearing'].append(0.0) # en radians
...
while True:
    ...
    thermal_ids_to_check = list(active_thermals.keys())
    
    #for each active thermal if not active regen another one
    for thermal_id in thermal_ids_to_check:
        thermal_data = active_thermals[thermal_id]
        if not thermal_data.is_active(current_time):
            # Régénérer un thermique si celui-ci n'est plus actif
            new_thermal = thermal_generator.generate_random_thermals(1, current_time, thermal_generated)[thermal_generated]
            active_thermals[thermal_id] = new_thermal
            thermal_generated += 1
    ...
    if FLT_track[0]['flight_mode'][-1] == 'soaring' and FLT_track[0].get('soaring_start_time'):
